...
class util_timer;   // 因为client_data与util_timer两个类互相引用，所以需要对任意一个类前向声明
...
// 将定时器对象存储在，定时器链表，它是一个升序、双向链表，且带有头节点和尾节点。操作效率高
...
public:
    ...
    void add_timer( util_timer* timer ) {
        ...
        /* 如果目标定时器的超时时间小于当前链表中所有定时器的超时时间(也就是head->expire)，则把该定时器插入链表头部,作为链表新的头节点，
    ...
    void adjust_timer(util_timer* timer)
    {
        ...
        util_timer* tmp = timer->next;//对timer的下一个节点保存为tmp
